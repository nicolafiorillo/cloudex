defmodule Cloudex.CloudinaryApi do
  ...
  def delete(item) when is_bitstring(item) do
    delete_file(item, %{type: :public_id})
  end
  ...
  @doc """
  Deletes all images with prefix in public id
  """
  @spec delete_resources(String.t) :: {:ok, %Cloudex.DeletedImage{}} | {:error, any}
  def delete_resources(item) when is_bitstring(item) do
    delete_file(item, %{type: :prefix})
  end
  ...
  defp delete_file(item, opts) do
    delete_mode = delete_file_type(opts)
    url = "#{@base_url}#{Cloudex.Settings.get(:cloud_name)}/resources/image/upload?#{delete_mode}=#{item}"
    case HTTPoison.delete(url, @cloudinary_headers, hackney_options()) do
      {:ok, _} -> delete_file_ret(item, opts)
      error    -> error
    end
  end

  defp delete_file_ret(item, %{type: :public_id}), do: {:ok, %Cloudex.DeletedImage{public_id: item}}
  defp delete_file_ret(item, %{type: :prefix}), do: {:ok, %Cloudex.DeletedImage{prefix: item}}

  @spec delete_file_type(map) :: String.t
  defp delete_file_type(%{type: :public_id}), do: "public_ids[]"
  defp delete_file_type(%{type: :prefix}), do: "prefix"
